...
export class AuthService implements OnModuleInit {
  ...
  async getProfile(firebaseUid: string) {
    try {
      ...
      this.logger.error('Get profile failed', error.stack, 'AuthService');
      throw new UnauthorizedException('Failed to retrieve user profile');
    }
  }

  async refreshSession(firebaseUid: string, firebaseToken: string) {
    try {
      const user = await this.prisma.user.findUnique({
        where: { firebaseUid },
      });
      ...
      this.logger.activity('SESSION_REFRESHED', user?.id || admin?.id, {
        sessionId: session.id,
      });
      ...
      this.logger.error('Session refresh failed', error.stack, 'AuthService');
